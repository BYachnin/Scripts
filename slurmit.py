...
def gen_varkey():
	...
	vars.append(('array', 'array', str, None, False, 'array', "Enter the number of array elements to include.  For example, an array with 10 subjobs numbered 1-10 should be given --array 1-10.  If you include this without --arraygen (ie. array of numbered elements), you must put 'percenta' in --outfiles/--log/--err and 'dollarjob' in --command.  If --arraygen is included, --array can be autogenerated, or alternatively a subset of indices can be specified."))
	vars.append(('arraygen', None, str, None, False, 'other', "Specify bash code to generate a bash array to be used to populate the SLURM array.  In bash, an array can be generated within parentheses, and specific elements can be referenced with square brackets.  For example, to populate the array with all files in a directory, use (mydir/*.pdb) ."))
...
def arg_logic(args, key):
	...
	#TODO: If args.usearray, check if args.array is empty, and autogenerate it if so.
